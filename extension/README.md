...
Each combination of backend + MCP servers + allowed directories is a **Preset** you build once and apply globally or per workspace. Mix and match freely — "Bedrock for client work, Ollama offline, OpenRouter for niche models" — and switch from the sidebar (shown above) or the status bar.
